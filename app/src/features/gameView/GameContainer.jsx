  ...
  componentDidMount() {
    ...
    this.stompClient.onConnect = function (_frame) {
      componentThis.stompClient.subscribe(
        `${TOPIC_GAME_PREFIX}/${componentThis.props.activeGameId}/${TOPIC_GAME_PLAYERS}`,
        (message) => {
      ...
      componentThis.stompClient.subscribe(
        `${TOPIC_GAME_PREFIX}/${componentThis.props.activeGameId}/${TOPIC_GAME_PAYMENT}`,
        (message) => {
          // called when the client receives a STOMP message from the server
          if (message.body) {
      ...
      componentThis.stompClient.subscribe(
        `${TOPIC_GAME_PREFIX}/${componentThis.props.activeGameId}/${TOPIC_GAME_PROPERTY_UPDATE}`,
        (message) => {
          // called when the client receives a STOMP message from the server
          if (message.body) {
