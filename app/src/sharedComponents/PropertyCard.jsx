...
const PropertyCard = ({
  ...
  const formattedRentForSite = rentForSite
    ? formatNumberAsCurrency(rentForSite)
    : "";
  const formattedRentForColorGroup = rentForColorGroup
    ? formatNumberAsCurrency(rentForColorGroup)
    : "";
  const formattedRentOneHouseOrRailroad = rentOneHouseOrRailroad
    ? formatNumberAsCurrency(rentOneHouseOrRailroad)
    : "";
  const formattedRentTwoHouseOrRailroad = rentTwoHouseOrRailroad
    ? formatNumberAsCurrency(rentTwoHouseOrRailroad)
    : "";
  const formattedRentThreeHouseOrRailroad = rentThreeHouseOrRailroad
    ? formatNumberAsCurrency(rentThreeHouseOrRailroad)
    : "";
  const formattedRentFourHouseOrRailroad = rentFourHouseOrRailroad
    ? formatNumberAsCurrency(rentFourHouseOrRailroad)
    : "";
      ...
      <CardBody>
        <Container>
          ...
          {isRegularProperty && (
            <Row>
              <Col>
                <div>Rent: {formattedRentForSite}</div>
                <div>
                  Rent if all color group owned (no houses):{" "}
                  {formattedRentForColorGroup}
                </div>
                <div>Rent w/ 1 House: {formattedRentOneHouseOrRailroad}</div>
                <div>Rent w/ 2 Houses: {formattedRentTwoHouseOrRailroad}</div>
                <div>Rent w/ 3 Houses: {formattedRentThreeHouseOrRailroad}</div>
                <div>Rent w/ 4 Houses: {formattedRentFourHouseOrRailroad}</div>
                <div>Rent w/ Hotel: {formattedRentHotel}</div>
              </Col>
            </Row>
          )}
          {isRailroad && (
          ...
          {isUtility && (
            <Row>
              <Col>
                <div>Rent w/ 1 Utility Owned: 4 times amount shown on dice</div>
                <div>
          ...
          <Row>
            <Col>
              <div>Mortgage Value: {formattedMortgageValue}</div>
              <div>Cost to Unmortgage: {formattedUnmortgageValue}</div>
            </Col>
          </Row>
          {ownedByPlayerName && ownedByPlayerName.length > 0 && (
            <Row>
              <Col>
                <div>Owned By: {ownedByPlayerName}</div>
              </Col>
            </Row>
          )}
          {(isShowBuyButton ||
            ...
            <Row>
              <Col>
                {isShowBuyButton && (
                  <Button
                    color="primary"
                    onClick={() => buyPropertyFunction(propertyClaimId)}
                  >
                    Buy
                  </Button>
                )}
                {isShowMortgageButton && (
                  <Button
                    color="primary"
                    onClick={() => mortgagePropertyFunction(propertyClaimId)}
                  >
                    Mortgage
                  </Button>
                )}
                {isShowUnmortgageButton && (
                  <Button
                    color="primary"
                    onClick={() => unmortgagePropertyFunction(propertyClaimId)}
                  >
                    Unmortgage
                  </Button>
                )}
