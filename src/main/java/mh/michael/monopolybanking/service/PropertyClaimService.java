...
public class PropertyClaimService {
    ...
    public PropertyClaimDTO mortgageProperty(
        ...
        if (propertyClaim.getIsMortgaged()) {
            log.error("User attempted to mortgage a property claim that is already mortgaged");
            throw new ResponseStatusException(HttpStatus.CONFLICT, "That property is already mortgaged");
        }
    ...
    public PropertyClaimDTO unmortgageProperty(
        ...
        if (!jwtUserDetails.getGameIdList().contains(gameId)) {
            log.error("User attempted to unmortgage property claim in game they don't have access to");
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Access Denied");
        }

        if (!jwtUserDetails.getPlayerIdList().contains(playerId)) {
            log.error("User attempted to unmortgage property claim as player they don't have access to");
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Access Denied");
        }
        ...
        if (propertyClaim.getOwnedByPlayer().getId() != playerId) {
            log.error("Player tried to unmortgage a property claim owned by another player");
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Access Denied");
        }

        if (!propertyClaim.getIsMortgaged()) {
            log.error("User attempted to unmortgage a property claim that is not mortgaged");
            throw new ResponseStatusException(HttpStatus.CONFLICT, "That property is not mortgaged");
        }
        ...
        PayRequestDTO payRequestDTO = PayRequestDTO.builder()
                .payRequestUUID(UUID.randomUUID().toString())
                .amountToPay(propertyClaim.getProperty().getUnmortgageValue())
                .isToSink(true)
                .isFromSink(false)
                .fromId(player.getId())
                .toId(bankMoneySink.getId())
        ...
        propertyClaimDTO.setIsUnmortgagingPropertyMsg(true);
